...
class AbstractMPSUnitCell:
    ...
    def random(cls,
        ...
        tensors = misc_mps.initialize_mps_tensors(
            initializer_function,
            D,
            d,
            dtype,
...
class AbstractFiniteMPS(AbstractMPSUnitCell):
    ...
    @classmethod
    def from_dense(cls, psi, max_bond_dim=None, auto_trunc_max_err=0.0, name=None):
        """Construct an MPS from a dense vector psi.
        The shape of psi must be the sequence of site Hilbert space dimensions.
        Args:
...
class AbstractInfiniteMPS(AbstractMPSUnitCell):
    ...
    def TMeigs_power_method(self,
        ...
        return misc_mps.TMeigs_power_method(tensors=tensors,
                                            direction=direction,
                                            init=init,
                                            precision=precision,
                                            nmax=nmax)
...
class MPSUnitCell_Generic(AbstractMPSUnitCell):
    ...
    def random(cls,
        ...
        *args,**kwargs: further arguments to initializer_function
        ...
        if not len(d) == (len(D) - 1):
            raise ValueError(
                'MPSUnitCell_Generic.random: len(d)! = len(D)-1!')

        kwargs['minval'] = kwargs.get('minval', -0.1)
        kwargs['maxval'] = kwargs.get('maxval', 0.1)
        return cls(tensors=misc_mps.initialize_mps_tensors(initializer_function,
                                                           D,d,dtype,
                                                           *args,**kwargs),
                   name=name)
...
class FiniteMPS_Generic(MPSUnitCell_Generic, AbstractFiniteMPS):
    ...
    def random(cls,
        ...
        *args,**kwargs: further arguments to initializer_function
        ...
        if not len(d) == (len(D) + 1):
            raise ValueError(
                'FiniteMPS_Generic.random: len(d)! = len(D)+1!')
        D = [1] + D + [1]
    ...
    def apply_2site(self, op, n, max_bond_dim=None, auto_trunc_max_err=0.0):
        ...
        if tf.executing_eagerly():
            ...
            if form_check > 10 * res:
                ...
                print("Warning: Now violating canonical form by at least {}".
                      format(form_check))
...
class MPSUnitCellCentralGauge(AbstractMPSUnitCell):
    ...
    def dtype(self):
        assert (np.all(
            [self._tensors[0].dtype == t.dtype for t in self._tensors]))
    ...
    def get_env_right(self, site):
        ...
        if site == len(self) - 1:
            ...
            r = misc_mps.ncon([self.centermatrix,
                          tf.conj(self.centermatrix)], [[-1, 1], [-2, 1]])
    ...
    def norm(self):
        return tf.sqrt(
            misc_mps.ncon([self.centermatrix,
                      tf.conj(self.centermatrix)], [[0, 1], [0, 1]]))
    ...
    def position(self, bond):
        ...
        if bond > self.pos:
            ...
            for n in range(self.pos, bond):
                ...
                self._tensors[n] = tensor
                if (n + 1) < bond:
                    self._tensors[n + 1] = misc_mps.ncon(
                        [self.mat, self._tensors[n + 1]],
                        [[-1, 1], [1, -2, -3]])
    ...
    def check_ortho(tensor, which='l', thresh=1E-8):
        ...
        """
        return MPSUnitCellCentralGauge.ortho_deviation(
            tensor, which).numpy().real < thresh
...
class FiniteMPSCentralGauge(MPSUnitCellCentralGauge,
                            AbstractFiniteMPS):
    """
    ...
    def canonize(self, name=None):
        ...
        return FiniteMPS_Schmidt(
            Gams=Gammas, Lams=Lambdas, name=name)
...
class InfiniteMPSCentralGauge(MPSUnitCellCentralGauge,
                              AbstractInfiniteMPS):
    """
    ...
    def random(cls,
        ...
        out.restore_form(numeig=numeig, precision=precision,power_method=power_method)
        return out
    ...
    def cycle(self, shift_by):
        ...
        new_center_matrix = misc_mps.ncon([new_center_matrix, self.mat],
                                     [[-1, 1], [1, -2]])
        tensors = (
            [self._tensors[n] for n in range(shift_by, len(self._tensors))
    ...
    def restore_form(self,
        ...
        tensors=[self.get_tensor(n) for n in range(len(self))]
        if not power_method:
            self._tensors,self.mat,self.connector,self._right_mat = misc_mps.restore_helper(tensors=tensors,
                                                                                            init=init,
                                                                                            precision=precision,
                                                                                            ncv=ncv,
                                                                                            nmax=nmax,
            ...
            self._tensors,self.mat,self.connector,self._right_mat = misc_mps.restore_helper_power_method(tensors=tensors,
                                                                                                         init=init,
                                                                                                         precision=precision,
                                                                                                         nmax=nmax,
                                                                                                         pinv=pinv)
    ...
    def get_left_orthogonal_imps(self,
        ...
        right_mat = tf.eye(int(D_end), dtype=dtype)
        position = len(self)
        return InfiniteMPSCentralGauge(tensors=[self.get_tensor(n) for n in range(len(self))],
                                       centermatrix=centermatrix,
                                       connector=connector,
                                       right_mat=right_mat,
                                       position=position,
    ...
    def get_right_orthogonal_imps(self,
        ...
        return InfiniteMPSCentralGauge(tensors=tensors,
                                       centermatrix=centermatrix,
                                       connector=connector,
                                       right_mat=right_mat,
                                       position=position,
