...
def mps_from_dense(psi, max_bond_dim=None, auto_trunc_max_err=0.0):
    ...
    while sites_remaining > 3:
        ...
        Lm1 = tf.diag(s_prev)
        psiR = misc_mps.ncon(
            [Lm1, psiR], [(-1, 0), (0, *range(-2, -sites_remaining - 1, -1))])
...
class AbstractMPSUnitCell:
    ...
    def expvals_1site(self, ops, sites):
        ...
        for n in range(len(sites)):
            ...
            expval = misc_mps.ncon(
                [l, A, op, r, tf.conj(A)], [(1, 3), (1, 0, 2), (4, 0), (2, 5),
                                            (3, 4, 5)])
            res.append(expval)
...
class AbstractFiniteMPS(AbstractMPSUnitCell):
    ...
    def to_dense(self):
        ...
        for j in range(1, N):
            ...
            psi = misc_mps.ncon([psi, A], [('l', 'p1', 'ri'),
                                           ('ri', 'p2', 'r')],
                                out_order=['l', 'p1', 'p2', 'r'],
                                con_order=['ri'])
...
class MPSUnitCell_Schmidt(AbstractMPSUnitCell):
    ...
    def check_form(self):
        ...
        for n in range(N):
            ...
            AR = misc_mps.ncon([Gams[n], Lams[(n + 1) % N]], [(-1, -2, 1),
                                                              (1, -3)])
...
class MPSUnitCellCentralGauge(AbstractMPSUnitCell):
    ...
    def normalize(self):
        ...
        """
        Z = tf.sqrt(
            misc_mps.ncon([self.mat, tf.conj(self.mat)], [[1, 2], [1, 2]]))
        self.mat /= Z
        return Z
