...
def compute_Hamiltonian_environments(mps,
    ...
    lb, hl = compute_steady_state_Hamiltonian_GMRES(
        ...
        right_dominant=ncon([mps.mat, tf.conj(mps.mat)],
                                 [[-1, 1], [-2, 1]]),
        precision=precision,
        nmax=nmax)
    ...
    rb, hr = compute_steady_state_Hamiltonian_GMRES(
        ...
        left_dominant=ncon([mps.mat, tf.conj(mps.mat)],
                                [[1, -1], [1, -2]]),
        precision=precision,
        nmax=nmax)
    return lb, rb, hl, hr
...
def apply_2site_schmidt_canonical_python(op,
    ...
    nop = net.add_node(
        op, axis_names=["p_out_1", "p_out_2", "p_in_1", "p_in_2"])
...
def apply_2site_generic_python(op, A1, A2, max_bond_dim=None, auto_trunc_max_err=0.0):
    """Applies a two-site local operator to an MPS.
...
def TMeigs_power_method(tensors,
    ...
    n_final, eta, state_final, diff=tf.while_loop(cond, do_step,(0,tf.cast(0.0, dtype), x, tf.cast(1.0, dtype.real_dtype)))
...
def initialize_mps_tensors_numpy(initializer_function, D, d, dtype,
    ...
    if np.issubdtype(dtype, np.floating):
        return [initializer_function([Ds[n], d[n], Ds[n + 1]]).astype(dtype)*(maxval-minval)+minval for n in range(len(d))
        ]
        
    elif np.issubdtype(dtype, np.complexfloating):
        return [initializer_function([Ds[n], d[n], Ds[n + 1]]).astype(dtype)*(maxval-minval)+minval+
                1.0j*initializer_function(
...
def initialize_mps_tensors(initializer_function,D,d,dtype,*args,**kwargs):
    ...
    *args,**kwargs:            additional arguments and key-word arguments to `initializer_function`
    """
    if np.issubdtype(dtype.as_numpy_dtype, np.floating):
        return [initializer_function(shape=[D[n], d[n], D[n + 1]],
                                     dtype=dtype,
                                     *args,
        ...
        return [
            tf.complex(
                initializer_function(shape=[D[n], d[n], D[n + 1]],
