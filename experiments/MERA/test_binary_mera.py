...
def test_ascending_descending(chi, dtype):
    ...
    np.testing.assert_allclose(np.array([energies[p] / energies[p + 1] for p in range(len(energies) - 1)]),0.5)

@pytest.mark.parametrize("chi", [4, 6])
@pytest.mark.parametrize("dtype", [tf.float64, tf.complex128]) 
def test_steady_state(chi, dtype):
    isometry = misc_mera.w_update_svd_numpy(np.random.rand(chi, chi, chi).astype(dtype.as_numpy_dtype))
    unitary = misc_mera.u_update_svd_numpy(np.random.rand(chi, chi, chi, chi).astype(dtype.as_numpy_dtype))
    rho = tf.reshape(tf.eye(chi * chi * chi, dtype=dtype), (chi, chi, chi, chi, chi, chi))
    rho_ss = bml.steady_state_density_matrix(nsteps=60, rho=rho, isometry=isometry, unitary=unitary)
...
def test_disentangler_envs(chi,dtype):
    ...
    t_a = [tn.ncon([envs[n],unitary],[[1,2,3,4],[1,2,3,4]]) for n in range(2)]
    t_b = [tn.ncon([envs[n],unitary],[[1,2,3,4],[1,2,3,4]]) for n in range(2,4)]
...
def test_isometry_envs(chi, dtype):
    ...
    t_b = [tn.ncon([envs[n],isometry],[[1,2,3],[1,2,3]]) for n in range(3,6)]    
    np.testing.assert_allclose(t_a, t_a[0])
    np.testing.assert_allclose(t_b, t_b[0])    
