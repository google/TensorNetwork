...
def w_update_svd_numpy(wIn):
    shape = wIn.shape
    ut, st, vt = np.linalg.svd(
        np.reshape(wIn, (shape[0] * shape[1], shape[2])), full_matrices=False)
    return -np.reshape(ncon.ncon([ut, vt], [[-1, 1], [1, -2]]), shape)
...
def left_ascending_super_operator(ham, isometry, unitary):
    ...
    hright = ncon.ncon([
        ...
        np.conj(isometry),
        np.conj(isometry),
        np.conj(isometry), unitary, unitary,
        np.conj(unitary),
        np.conj(unitary), ham
    ], [
...
def unlock_layer(wC, uC, noise=0.0):
    ...
    wC[-1] += (np.random.random_sample(shape=wC[-1].shape).astype(wC[-1].dtype)
               - 0.5) * noise
    uC[-1] += (np.random.random_sample(shape=uC[-1].shape).astype(uC[-1].dtype)
               - 0.5) * noise
    return wC, uC
...
def initialize_binary_MERA(phys_dim, chi, dtype=np.float64):
    """
    ...
    while True:
        wC.append(
            np.reshape(
                np.eye(phys_dim**(2**(n + 1)), min(phys_dim**(2**(n + 1)),
                                                   chi)).astype(dtype),
                (phys_dim**(2**n), phys_dim**(2**n),
                 min(phys_dim**(2**(n + 1)), chi))))
        uC.append(
            np.reshape(
                np.eye(phys_dim**(2**(n + 1))).astype(dtype),
                (phys_dim**(2**n), phys_dim**(2**n), phys_dim**(2**n), phys_dim
                 **(2**n))))
        n += 1
    ...
    rho = np.reshape(
        np.eye(chi_top * chi_top * chi_top).astype(dtype=dtype),
