...
# def test_eigsh_all_eigvals_with_init(dtype, which):
#   backend = jax_backend.JaxBackend()
#   D = 16
#   np.random.seed(10)
#   init = backend.randn((D,), dtype=dtype, seed=10)
...
#   eta, U = backend.eigsh(mv, [H], init, numeig=D, num_krylov_vecs=D, which=which)
#   eta_exact, U_exact = np.linalg.eigh(H)
#   compare_eigvals_and_eigvecs(
#       np.stack(U, axis=1), eta, U_exact, eta_exact, thresh=1E-8)
...
# @pytest.mark.parametrize("dtype", [np.float64, np.complex128])
# @pytest.mark.parametrize("which", ["LR", "LM"])
# def test_eigsh_all_eigvals_no_init(dtype, which):
#   backend = jax_backend.JaxBackend()
#   D = 16
#   np.random.seed(10)
#   H = backend.randn((D, D), dtype=dtype, seed=10)
...
# @pytest.mark.parametrize("dtype", [np.float64, np.complex128])
# @pytest.mark.parametrize("which", ["LR", "LM"])
# def test_eigsh_few_eigvals_with_init(dtype, which):
#   backend = jax_backend.JaxBackend()
#   D = 16
#   np.random.seed(10)
#   init = backend.randn((D,), dtype=dtype, seed=10)
