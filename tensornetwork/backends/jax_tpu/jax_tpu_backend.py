...
class JaxTPUTensor(base_backend.BaseBackend):
  def __init__(self, concrete_tensor, virtual_tensor):
    ...
    if onp.prod(concrete_tensor.shape) != onp.prod(virtual_tensor.shape):
      raise ValueError(
          "Virtual shapes and concrete shapes do not math."
          "Virtual shape: {}, concrete shape: {}".format(
              virtual_tensor.shape,
              concrete_tensor.shape))
...
class JaxTPUBackend(base_backend.BaseBackend):
  ...
  def tensordot(self, a, b, axes):
    ...
    new_virtual = self.shell_backend.tensordot(
        a.virtual_tensor, b.virtual_tensor, axes)
...
def optimized_dot_general(a, b, contracting_axes):
  ...
  for i, tensor in enumerate([a, b]):
    ...
    new_shape = list(tensor.shape[:tensor.first_expensive_axis]) + [tensor.concrete_tensor.shape[-1]]
    ...
    tmp_tensor = jax.numpy.transpose(tmp_tensor, perm)
    tmp_tensor = jax.numpy.reshape(tmp_tensor, [new_dim] + list(tmp_tensor.shape[len(axes):]))
