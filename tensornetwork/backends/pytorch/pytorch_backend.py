...
class PyTorchBackend(base_backend.BaseBackend):
  ...
  def eye(self, N: int, M: Optional[int], dtype: Optional[torch.dtype]=torch.float64) -> Tensor:
    return self.torch.eye(n=N, m=M, dtype=dtype)
  
  def ones(self, shape: Tuple[int], dtype: Optional[torch.dtype]=torch.float64) -> Tensor:
    return self.torch.ones(shape, dtype=dtype)
  
  def zeros(self, shape: Tuple[int], dtype: Optional[torch.dtype]=torch.float64) -> Tensor:
    return self.torch.zeros(shape, dtype=dtype)
  
  def randn(self, shape: Tuple[int], dtype: Optional[torch.dtype]=torch.float64) -> Tensor:
