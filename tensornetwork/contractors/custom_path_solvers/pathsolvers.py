...
def greedy_size_solve(log_adj_in: np.ndarray):
  ...
  for t in range(N-1):
    ...
    dim_change = (1e6*(new_dims - max_dim)).astype(int)
    ...
    all_costs = np.array([single_cost[xcoord[i],ycoord[i]] 
                          for i in range(len(xcoord))])
    cont_dims = np.array([log_adj[xcoord[i],ycoord[i]] 
                          for i in range(len(xcoord))])
...
def _full_solve_single(log_adj: np.ndarray,
  ...
  for i in range(1,N):
    for j in range(i):
      ...
      if num_kept > 0:
        ...
        next_orders = np.vstack([j*np.ones(len(new_pos),dtype=int),
        ...
        stable_pos = final_dims < (np.maximum(dims[j,:],dims[i,:])+0.001)  
        ...
        if final_costs.size > mid_kept:
          temp_pos, num_truncated = _reduce_nets(final_costs,
                                                 final_groups,
                                                 final_stable,
  ...
  if only_outer_exist: # network contains only outer products
    ...
    return _full_solve_single(log_adj,costs,groups,orders,cost_bound=cost_bound,
                             max_branch=max_branch,allow_outer=True)
  else: 
    # compress outputs
...
def _reduce_nets(costs: np.ndarray,
  ...
  if max_branch is not None:
    if (orig_kept > max_branch):
      ...
      temp_pos = np.concatenate((np.arange(num_cheapest),stable_pos[:min(
        len(stable_pos),num_stable)] + num_cheapest),axis=0)
      new_pos = new_pos[temp_pos]
