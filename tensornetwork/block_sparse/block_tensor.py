...
def fuse_stride_arrays(dims: Union[List[int], np.ndarray],
  ...
  return fuse_ndarrays([
      np.arange(0, strides[n] * dims[n], strides[n], dtype=np.int64)
...
def _find_diagonal_sparse_blocks(
  ...
  if partition in (0, num_inds):
    ...
    block_maps = [np.arange(0, num_nonzero, dtype=np.int64).ravel()]
...
def _find_transposed_diagonal_sparse_blocks(
  ...
  if orig_num_blocks == 0:
    ...
    return [], obj, np.array([], dtype=np.int64)
  ...
  if tr_partition == 0:
    ...
    block_qnums, new_row_map, new_col_map = intersect(
        ...
        axis=1,
        return_indices=True)
    block_dims = np.array([[1], new_col_degen[new_col_map]], dtype=np.int64)
    num_blocks = 1
    ...
    block_qnums, new_row_map, new_col_map = intersect(
        ...
        axis=1,
        return_indices=True)
    block_dims = np.array([new_row_degen[new_row_map], [1]], dtype=np.int64)
    num_blocks = 1
