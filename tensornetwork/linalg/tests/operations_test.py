...
@pytest.mark.parametrize("dtype", np_float_dtypes)
def test_pivot(backend, dtype, pivot_axis):
  shape = (2, 2, 2, 2, 2)
  A, _ = safe_randn(shape, backend, dtype)
  if A is not None:
    np.testing.assert_allclose(
        tensornetwork.linalg.operations.pivot(A, pivot_axis=pivot_axis).array,
        A.backend.pivot(A.array, pivot_axis=pivot_axis))


@pytest.mark.parametrize("pivotA", [-1, 1, 2])
@pytest.mark.parametrize("pivotB", [-1, 1, 2])
@pytest.mark.parametrize("dtype", np_not_half)
def test_kron(backend, dtype, pivotA, pivotB):
  shapeA = (2, 2, 2)
  shapeB = (2, 2, 2, 2)
  ...
  if A is not None and B is not None:
    tkron = tensornetwork.linalg.operations.kron(A, B, pivot_axisA=pivotA,
                                                 pivot_axisB=pivotB)
    pA = initA.reshape((np.prod(shapeA[:pivotA]), np.prod(shapeA[pivotA:])))
    pB = initB.reshape((np.prod(shapeB[:pivotB]), np.prod(shapeB[pivotB:])))
    nkron = np.kron(pA, pB).reshape((2, 2, 2, 2, 2, 2, 2))
    atol = nkron.size * np.finfo(dtype).eps
    np.testing.assert_allclose(nkron, tkron.array, atol=atol)
